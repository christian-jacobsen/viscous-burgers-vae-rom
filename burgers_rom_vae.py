...
class _DenseBlock_1d(nn.Sequential):
    def __init__(self, n_layers, in_features, growth_rate, act=nn.ReLU(inplace=True)):
        ...
        for i in range(n_layers):
            layer = _DenseBlockLayer_1d(in_features + i*growth_rate, growth_rate, act)
...
class _DecodeBlock_1d(nn.Sequential):
    def __init__(self, in_features, out_features, act=nn.ReLU(inplace=True)):
        ...
        self.add_module('norm2', nn.BatchNorm1d(out_features))
        self.add_module('act2', act)
        self.add_module('upsample', nn.Upsample(scale_factor=2, mode='linear', align_corners=False))
...
class burgers_rom_vae(nn.Module):
    def __init__(self, n_latent, act = nn.ReLU()):
        ...
        self.E1_m.add_module('initial_conv', nn.Conv1d(data_channels, initial_features, 
            kernel_size = 7, stride = 2, padding = 2, bias=False))

        n_features = initial_features

        n_layers = 4
        block = _DenseBlock_1d(n_layers = n_layers, in_features = n_features, 
                growth_rate = K, act=act)
        self.E1_m.add_module('dense_block1', block)
        n_features = n_features + n_layers*K
        enc = _EncodeBlock_1d(in_features = n_features, out_features = n_features // 2, act=act)
        self.E1_m.add_module('encode_block1', enc)
        n_features = n_features // 2
        
        flatten_dim = 32*n_features
        self.E1_m.add_module('flatten', nn.Flatten())
        self.E1_m.add_module('fullconn1', nn.Linear(flatten_dim, flatten_dim//2))
        self.E1_m.add_module('act1', act)
        self.E1_m.add_module('fullconn2', nn.Linear(flatten_dim//2, n_latent))
        ...
        block = _DenseBlock_1d(n_layers = n_layers, in_features = n_features, 
                growth_rate = K, act=act)
        self.E1_lv.add_module('dense_block1', block)
        n_features = n_features + n_layers*K
        enc = _EncodeBlock_1d(in_features = n_features, out_features = n_features // 2, act=act)
        self.E1_lv.add_module('encode_block1', enc)
        n_features = n_features // 2
        
        flatten_dim = 32*n_features
        self.E1_lv.add_module('flatten', nn.Flatten())
        self.E1_lv.add_module('fullconn1', nn.Linear(flatten_dim, flatten_dim//2))
        self.E1_lv.add_module('act1', act)
        self.E1_lv.add_module('fullconn2', nn.Linear(flatten_dim//2, n_latent))

        # Decoder1 Mean
        self.D1_m.add_module('fullconn1', nn.Linear(n_latent, flatten_dim//2))
        self.D1_m.add_module('act1', act)
        self.D1_m.add_module('fullconn2', nn.Linear(flatten_dim//2, flatten_dim))
        self.D1_m.add_module('reshape', _Reshape_1d((-1, n_features, 32)))
        
        n_layers = 6
        block = _DenseBlock_1d(n_layers=n_layers, in_features = n_features, growth_rate = K, act=act)
        self.D1_m.add_module('dense_block1', block)
        n_features += n_layers*K
        dec = _DecodeBlock_1d(in_features = n_features, out_features = n_features//2, act=act)
        self.D1_m.add_module('decode_block1', dec)
        n_features = n_features//2
        n_layers = 4
        block = _DenseBlock_1d(n_layers=n_layers, in_features = n_features, growth_rate = K, act=act)
        self.D1_m.add_module('dense_block2', block)
        n_features += n_layers*K
        dec = _DecodeBlock_1d(in_features = n_features, out_features = data_channels, act=act)
        self.D1_m.add_module('decode_block2', dec)
