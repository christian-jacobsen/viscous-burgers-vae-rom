...
for i in range(n_ic):
    ...
    U[:,0] = u0
    for j in range(nx-1):
        U[:,j+1] = np.roll(U[:,j], 1)#RK4(u0, dx, dt, F, nsteps)
    T = x0
...
plt.plot(x0,U[:,0])
plt.plot(x0,U[:,-1])
plt.savefig("./data/LinAdv1D/linadv1d_snapshots.png")
...
ax.set_zlabel('u')
plt.savefig("./data/LinAdv1D/linadv1d_surf.png")
plt.figure(3)
plt.imshow(U, cmap='jet')
plt.savefig("./data/LinAdv1D/linadv1d_field.png")

f1 = h5py.File("./data/LinAdv1D/linadv1d_ic_{}.hdf5".format(n_ic),"w")
